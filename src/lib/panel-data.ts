...
export async function getPedidosDelLocal(
  ...
  const { data, error } = await supabase
    .from("pedidos")
    .select(
      "id, local_id, usuario_id, items, subtotal, total, horario_retiro, estado, estado_revision, motivo_revision, desfasaje_precio, especificaciones, verificacion, nombre_cliente, telefono_cliente, alias_cliente, created_at"
    )
    .eq("local_id", localId)
